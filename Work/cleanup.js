...
let organizationParser = async (json) => {
    ...
    // Initialize the new organization object with the RID property
       ...
       if (template.hasOwnProperty(property)) {
          switch (template[property].type) {
            ...
            case 'multiple':
               ...
               for (let item of propertyObject) {
                  let temporal = {};
                  for (let firstLevel in item) {
                     if (firstLevel === 'atts') {
                        for (let attribute in item.atts) {
                           temporal[attribute] = item.atts[attribute];
                        } // end for
                        continue;
                     } // end if

                     if (typeof item[firstLevel] === 'string') {
                        temporal[firstLevel] = item[firstLevel];
                        continue;
                     } // end if

                     if (Array.isArray(item[firstLevel])) {
                        for (nestedItem of item[firstLevel]) {
                           if (typeof nestedItem === 'string') {
                              temporal[firstLevel] = item[firstLevel];
                           } else {
                              temporal[firstLevel] = [];
                              if (nestedItem.hasOwnProperty('atts')) {
                                 for (nestedAttr in nestedItem) {
                                    temporal[firstLevel][nestedAttr] = nestedItem[nestedAttr];
                                 } // end for
                              } // end if
                           } // end if - else
                        } // end for
                     ...
                     if (typeof item[firstLevel] === 'object' ) {
                        // Initialize the property
                        temporal[firstLevel] = {};
                        if (item[firstLevel].hasOwnProperty('atts')) {
                           for (let attributes in item[firstLevel]['atts']) {
                              temporal[firstLevel][attributes] = item[firstLevel]['atts'][attributes];
                           } // end for
                           temporal[firstLevel]['value'] = item[firstLevel]['value'];
                        } else {
                           temporal[firstLevel] = item[firstLevel]['value'];
                        } // end if - ele
                     } // end if
                  } // end for
